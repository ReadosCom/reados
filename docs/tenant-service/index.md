...
- Act as the discovery layer behind a dedicated login host such as `app.reados.localhost`.
...
6. Reados redirects the user to that tenant's login UI at `<tenant>.<root_fqdn>/authentication` (for example `demo.reados.localhost/authentication`).
...
- A host such as `app.reados.localhost` is a separate application from tenant-local applications.
